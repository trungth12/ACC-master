  ...
  class Server < CASServer::Base
    ...
    before do
      ...
      @template_engine = settings.config[:template_engine] || :erb
	    @default_service = 'http://acc.hpu.edu.vn'
		#@default_service = 'http://10.1.0.238:3004'
    ...
    get "#{uri_path}/login" do
      ...
      begin
        if @service
          ...
          #$LOG.info("Proceeding with CAS login without a target service.")
		      @service = "#{@default_service}/redirect"
        end
      ...
      if params.has_key? 'onlyLoginForm'
        ...
        if @form_action
          ...
          render @template_engine, :login, :layout => false
        end
      else
        render @template_engine, :login, :layout => false
      end
    ...
    post "#{uri_path}/login" do
      ...
      if error = validate_login_ticket(@lt)
        ...
        status 500
        return render @template_engine, :login, :layout => false
      end
